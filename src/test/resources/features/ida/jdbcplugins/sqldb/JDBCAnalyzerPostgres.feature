Feature:Verification of JDBC Analyzer using Postgres database and plugin validation
  Description: MLP-23777 - QA - PostgreSQL Data Source and View Support for Cataloger

  @precondition
  Scenario: MLP-23777:SC1#Update credential payload json for PostgressDB
    Given User update the below "Postgres Credentials" in following files using json path
      | filePath                                                         | username    | password    |
      | ida/postgressPayloads/credentials/postgressValidCredentials.json | $..userName | $..password |
  ...
  Scenario Outline: Configure the Credentials for PostgressDBDatasource
    ...
    Examples:
      ...
      | IDC         | TestSystemUser | application/json |       |       | Put  | settings/credentials/ValidPostgressCredentials     | ida/postgressPayloads/Credentials/postgressValidCredentials.json   | 200           |                  |          |
  ...
  Scenario:SC02#Verify whether the background of the panel is displayed in red when connection is unsuccessful due to invalid / Empty credentials in Local Node
    ...
    And user "enter text" in Add Data Source Page
      ...
      | URL       | jdbc:postgresql://dechewindock01v.asg.com:5432/postgres |
    And user "select dropdown" in Add Data Source Page
      | fieldName  | attribute                     |
...
#6938866
  ...
  Scenario Outline:MLP-23777:SC10#Run the Plugin configurations for DataSource and PostgresDBCataloger
    ...
    Examples:
      | ServiceName | ServiceUser    | Header           | Query | Param | type         | url                                                                                    | body                                                                       | response code | response message         | jsonPath                                              |
      | IDC         | TestSystemUser | application/json | raw   | false | Put          | settings/analyzers/PostgreSQLDBDataSource                                              | ida/postgressPayloads/DataSource/PostgressValidDataSourceConfig.json       | 204           |                          |                                                       |
  ...
  Scenario: SC#11: Verify the Host should have the appropriate metadata information in IDC UI and Database
    ...
    And user performs "item click" on "dechewindock01v.asg.com" item from search results
    Then the following metadata values should be displayed
      | metaDataAttribute | metaDataValue           | widgetName  |
      | Host name         | dechewindock01v.asg.com | Description |
  ...
  @webtest @MLP-23777 @sanity @positive @regression
  Scenario: SC#13: Verify User defined data types will be cataloged as UDT and user defined type domain is cataloged as DISTINCT
    Given User launch browser and traverse to login page
    And user enter credentials for "System Administrator1" role
    And user enters the search text "PostgressTag1" and clicks on search
    And user performs "facet selection" in "PostgressTag1" attribute under "Tags" facets in Item Search results page
    ...
    And user performs "facet selection" in "bug [Table]" attribute under "Hierarchy" facets in Item Search results page
    And user performs "facet selection" in "Column" attribute under "Metadata Type" facets in Item Search results page
  ...
  Scenario: SC#16: Verify the Postgres Table should not have constraints window if the table is not having any constraints.
    ...
    And user enter credentials for "System Administrator1" role
    And user enters the search text "PostgressTag1" and clicks on search
    And user enters the search text "postgres_tag_details [Table]" and clicks on search
    And user performs "facet selection" in "postgres_tag_details [Table]" attribute under "Hierarchy" facets in Item Search results page
  ...
  Scenario: SC17# Verify the technology tags got assigned to all PostgressDB items like Cluster,Service,Database...etc
    ...
    Then user "verify tag item presence" of technology tags for the below Type and file names
      ...
      | Default     | Cluster    | Metadata Type | PostgreSQL,PostgressTag1,Postgress_BA | dechewindock01v.asg.com | Postgress_BA |
      | Default     | Host       | Metadata Type | PostgreSQL,PostgressTag1,Postgress_BA | dechewindock01v.asg.com | Postgress_BA |
  ...
  Scenario:SC#18: Verify PostgressDBCataloger scans and collects data if schema name and table names are not provided in filters
    ...
    And user connect to the database and execute query for the following parameters
      ...
      | POSTGRES     | STRUCTURED   | json/IDA.json | postgresQueries | getPostgressSchemas | schema_name | returnstringlist | resultsInList |
    ...
    And user connect to the database and execute query for the following parameters
      | dataBaseName | dataBaseType | queryPath     | queryPage       | queryField                 | columnName | queryOperation   | storeResults  |
      | POSTGRES     | STRUCTURED   | json/IDA.json | postgresQueries | getPostgressTablesInSchema | table_name | returnstringlist | resultsInList |
  ...
  Scenario:SC19#Verify log entries/log enhancements(processed Items widget and Processed count) check for PostgressDBCataloger plugin logs.
    ...
    Then Analysis log "cataloger/PostgreSQLDBCataloger/PostgreCataloger%" should display below info/error/warning
      ...
      | INFO | Plugin Name:PostgreSQLDBCataloger, Plugin Type:cataloger, Plugin Version:1.1.0.SNAPSHOT, Node Name:LocalNode, Host Name:64918117aadc, Plugin Configuration name:PostgreCataloger                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | ANALYSIS-0071 | PostgreSQLDBCataloger | Plugin Version |
      ...
      | INFO | Plugin PostgreSQLDBCataloger Start Time:2020-06-15 23:43:21.828, End Time:2020-06-15 23:43:23.224, Processed Count:2, Errors:0, Warnings:0                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | ANALYSIS-0072 | PostgreSQLDBCataloger |                |
...
#    And user clicks on logout button
  ...
  Scenario Outline: SC23#user retrieves the total items for a catalog and copy to a json file
    ...
    Examples:
      ...
      | APPDBPOSTGRES | Default | dechewindock01v.asg.com              |      | response/postgressDB/actual/itemIds.json | $..Cluster.id               |
  ...
  Scenario: SC#25: Verify JDBC cataloger scans and collects data if multiple schema name alone is provided in filters
    ...
    And user connect to the database and execute query for the following parameters
      | dataBaseName | dataBaseType | queryPath     | queryPage       | queryField                  | columnName | queryOperation   | storeResults  |
      | POSTGRES     | STRUCTURED   | json/IDA.json | postgresQueries | getPostgressTablesInSchema1 | table_name | returnstringlist | resultsInList |
    And user "verifies" the "Tables" Item view page result "list" value with Postgres DB
  ...
  Scenario: SC#31-Configure the Postgress Datasource in internal node
    Given Execute REST API with following parameters
      | Header           | Query | Param | type | url                                       | body                                                                             | response code | response message | jsonPath                         |
      | application/json | raw   | false | Put  | settings/analyzers/PostgreSQLDBDataSource | ida/postgressPayloads/DataSource/PostgressInternalNodeValidDataSourceConfig.json | 204           |                  |                                  |
 ...
 # PostgressDBAnalyzer Cases #
  ...
  @positve @regression @sanity @webtest
  Scenario:SC36#Verify data profiling does not happen for complex data types in postgress Table/View.
    Given User launch browser and traverse to login page
    When user enter credentials for "System Administrator1" role
    And user enters the search text "DiffDataTypes [Table]" and clicks on search
    And user performs "facet selection" in "PostgressTag1" attribute under "Tags" facets in Item Search results page
    And user performs "facet selection" in "DiffDataTypes [Table]" attribute under "Hierarchy" facets in Item Search results page
    And user performs "facet selection" in "Column" attribute under "Metadata Type" facets in Item Search results page
    And user performs "item click" on "boxtype" item from search results
    And user "widget non presence" on "Most frequent values" in Item view page
    And user "verify metadata properties" section does not have the following values
      | metaDataAttribute | widgetName |
      | Last analyzed at  | Lifecycle  |
    And user enters the search text "DiffDataTypesView [Table]" and clicks on search
    And user performs "facet selection" in "PostgressTag1" attribute under "Tags" facets in Item Search results page
  ...
  @positve @regression @sanity @webtest
  Scenario:SC37#Verify the data profiling metadata information for numeric datatypes in Postgress table.
    Given User launch browser and traverse to login page
    When user enter credentials for "System Administrator1" role
    And user enters the search text "DiffDataTypesMinimized [Table]" and clicks on search
    And user performs "facet selection" in "PostgressTag1" attribute under "Tags" facets in Item Search results page
  ...
  Scenario:SC39#Verify the data profiling metadata information for Data,time and timestamp datatypes in postgress table.
    ...
    Then the following metadata values should be displayed
      ...
      | Minimum value                 | 05:19:20      | Statistics  |
      | Number of non null values     | 5             | Statistics  |
      | Percentage of non null values | 100           | Statistics  |
      | Number of null values         | 0             | Statistics  |
      | Number of unique values       | 5             | Statistics  |
  ...
  Scenario Outline:MLP-23777:SC41#Run the Plugin configurations for DataSource and PostgresDBCataloger
    ...
    Examples:
      | ServiceName | ServiceUser    | Header           | Query | Param | type         | url                                                                                       | body                                                                                   | response code | response message                 | jsonPath                                              |
      | IDC         | TestSystemUser | application/json | raw   | false | Put          | settings/analyzers/PostgreSQLDBDataSource                                                 | ida/postgressPayloads/DataSource/PostgressInternalNodeValidDataSourceConfig.json       | 204           |                                  |                                                       |
      ...
      | IDC         | TestSystemUser | application/json | raw   | false | Put          | settings/analyzers/PostgreSQLDBCataloger                                                  | ida/postgressPayloads/PluginConfiguration/PostgressConfigWithNoFilterInternalNode.json | 204           |                                  |                                                       |
      | IDC         | TestSystemUser | application/json | raw   | false | Get          | settings/analyzers/PostgreSQLDBCataloger                                                  |                                                                                        | 200           | PostgreCataloger                 |                                                       |
  ...
  Scenario:SC#62:Verify Tag is set for the column when match empty is true and all the column values in DB are empty.
    Given Tag verification of UI items in API for all the DataTypes
      ...
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_ALLEMPTY | email     | PostgressTag1,Postgress_BA,PostgreSQL,Email Address | ColumnQuerywithSchema | TagAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_ALLEMPTY | ipaddress | PostgressTag1,Postgress_BA,PostgreSQL,IP Address    | ColumnQuerywithSchema | TagAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_ALLEMPTY | ssn       | PostgressTag1,Postgress_BA,PostgreSQL,SSN           | ColumnQuerywithSchema | TagAssigned |
...
#######################################################################################################################################
  ...
  (Ex: 0.2 - 2 or more rows should have matcning column values)- Match Empty is False the column when match empty is true and all the column values in DB are empty.
    Given Tag verification of UI items in API for all the DataTypes
      | ClusterName             | ServiceName | DatabaseName | SchemaName | TableName/Filename                   | Column    | Tags                                                | Query                 | Action      |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_Ratiolessthan05EmptyFalse | email     | PostgressTag1,Postgress_BA,PostgreSQL,Email Address | ColumnQuerywithSchema | TagAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_Ratiolessthan05EmptyFalse | ipaddress | PostgressTag1,Postgress_BA,PostgreSQL,IP Address    | ColumnQuerywithSchema | TagAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_Ratiolessthan05EmptyFalse | ssn       | PostgressTag1,Postgress_BA,PostgreSQL,SSN           | ColumnQuerywithSchema | TagAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_Ratiolessthan05EmptyFalse | full_name | PostgressTag1,Postgress_BA,PostgreSQL,Full Name     | ColumnQuerywithSchema | TagAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_Ratiolessthan05EmptyFalse | gender    | PostgressTag1,Postgress_BA,PostgreSQL,Gender        | ColumnQuerywithSchema | TagAssigned |
  ...
  Scenario:SC#64: Verify Tag is set for the column when typePattern(String) and dataPattern/minimumRatio matches with the column type/value ratio
    Given Tag verification of UI items in API for all the DataTypes
      | ClusterName             | ServiceName | DatabaseName | SchemaName | TableName/Filename                  | Column    | Tags                                                | Query                 | Action      |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_RatioEqualTo05EmptyFalse | email     | PostgressTag1,Postgress_BA,PostgreSQL,Email Address | ColumnQuerywithSchema | TagAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_RatioEqualTo05EmptyFalse | ipaddress | PostgressTag1,Postgress_BA,PostgreSQL,IP Address    | ColumnQuerywithSchema | TagAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_RatioEqualTo05EmptyFalse | ssn       | PostgressTag1,Postgress_BA,PostgreSQL,SSN           | ColumnQuerywithSchema | TagAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_RatioEqualTo05EmptyFalse | full_name | PostgressTag1,Postgress_BA,PostgreSQL,Full Name     | ColumnQuerywithSchema | TagAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_RatioEqualTo05EmptyFalse | gender    | PostgressTag1,Postgress_BA,PostgreSQL,Gender        | ColumnQuerywithSchema | TagAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_ALLMATCH                 | email     | PostgressTag1,Postgress_BA,PostgreSQL,Email Address | ColumnQuerywithSchema | TagAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_ALLMATCH                 | ipaddress | PostgressTag1,Postgress_BA,PostgreSQL,IP Address    | ColumnQuerywithSchema | TagAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_ALLMATCH                 | ssn       | PostgressTag1,Postgress_BA,PostgreSQL,SSN           | ColumnQuerywithSchema | TagAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_ALLMATCH                 | full_name | PostgressTag1,Postgress_BA,PostgreSQL,Full Name     | ColumnQuerywithSchema | TagAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_ALLMATCH                 | gender    | PostgressTag1,Postgress_BA,PostgreSQL,Gender        | ColumnQuerywithSchema | TagAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_ALLEMPTY                 | email     | PostgressTag1,Postgress_BA,PostgreSQL,Email Address | ColumnQuerywithSchema | TagAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_ALLEMPTY                 | ipaddress | PostgressTag1,Postgress_BA,PostgreSQL,IP Address    | ColumnQuerywithSchema | TagAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_ALLEMPTY                 | ssn       | PostgressTag1,Postgress_BA,PostgreSQL,SSN           | ColumnQuerywithSchema | TagAssigned |
 ...
 #######################################################################################################################################
  ...
  Scenario:SC#66: Verify Tag is set for the column when namePattern(String) and dataPattern/minimumRatio matches with the column type/value ratio
    Given Tag verification of UI items in API for all the DataTypes
      ...
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_ALLEMPTY                 | ipaddress | PostgressTag1,Postgress_BA,PostgreSQL,IP Address    | ColumnQuerywithSchema | TagAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_ALLEMPTY                 | ssn       | PostgressTag1,Postgress_BA,PostgreSQL,SSN           | ColumnQuerywithSchema | TagAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_ALLEMPTY                 | full_name | PostgressTag1,Postgress_BA,PostgreSQL,Full Name     | ColumnQuerywithSchema | TagAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_ALLEMPTY                 | gender    | PostgressTag1,Postgress_BA,PostgreSQL,Gender        | ColumnQuerywithSchema | TagAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_ALLMATCH                 | email     | PostgressTag1,Postgress_BA,PostgreSQL,Email Address | ColumnQuerywithSchema | TagAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_ALLMATCH                 | ipaddress | PostgressTag1,Postgress_BA,PostgreSQL,IP Address    | ColumnQuerywithSchema | TagAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_ALLMATCH                 | ssn       | PostgressTag1,Postgress_BA,PostgreSQL,SSN           | ColumnQuerywithSchema | TagAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_ALLMATCH                 | full_name | PostgressTag1,Postgress_BA,PostgreSQL,Full Name     | ColumnQuerywithSchema | TagAssigned |
...
##################################################################################################################################
  ...
  Scenario:SC#69: Verify Tag is not set for the column when typePattern(other than String) and dataPattern/minimumRatio matches with the column type/value ratio in postgress table.
    Given Tag verification of UI items in API for all the DataTypes
      | ClusterName             | ServiceName | DatabaseName | SchemaName | TableName/Filename                  | Column    | Tags          | Query                 | Action         |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_ALLMATCH                 | email     | Email Address | ColumnQuerywithSchema | TagNotAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_ALLMATCH                 | ipaddress | IP Address    | ColumnQuerywithSchema | TagNotAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_ALLMATCH                 | ssn       | SSN           | ColumnQuerywithSchema | TagNotAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_ALLMATCH                 | full_name | Full Name     | ColumnQuerywithSchema | TagNotAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_ALLMATCH                 | gender    | Gender        | ColumnQuerywithSchema | TagNotAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_ALLEMPTY                 | email     | Email Address | ColumnQuerywithSchema | TagNotAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_ALLEMPTY                 | ipaddress | IP Address    | ColumnQuerywithSchema | TagNotAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_ALLEMPTY                 | ssn       | SSN           | ColumnQuerywithSchema | TagNotAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_ALLEMPTY                 | full_name | Full Name     | ColumnQuerywithSchema | TagNotAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_ALLEMPTY                 | gender    | Gender        | ColumnQuerywithSchema | TagNotAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_RatioEqualTo05EmptyFalse | email     | Email Address | ColumnQuerywithSchema | TagNotAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_RatioEqualTo05EmptyFalse | ipaddress | IP Address    | ColumnQuerywithSchema | TagNotAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_RatioEqualTo05EmptyFalse | ssn       | SSN           | ColumnQuerywithSchema | TagNotAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_RatioEqualTo05EmptyFalse | full_name | Full Name     | ColumnQuerywithSchema | TagNotAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_RatioEqualTo05EmptyFalse | gender    | Gender        | ColumnQuerywithSchema | TagNotAssigned |
  ...
  (Ex: 0.6 - 6 or more rows should have matcning column values including empty) - Match Empty is False -10 rows , 3 rows empty, 4 rows match,3 rows does not match
    Given Tag verification of UI items in API for all the DataTypes
      | ClusterName             | ServiceName | DatabaseName | SchemaName | TableName/Filename                          | Column    | Tags                                                | Query                 | Action         |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_Ratiogreaterthan05EmptyFalseTrue | ssn       | SSN                                                 | ColumnQuerywithSchema | TagNotAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_Ratiogreaterthan05EmptyFalseTrue | full_name | Full Name                                           | ColumnQuerywithSchema | TagNotAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_Ratiogreaterthan05EmptyFalseTrue | gender    | Gender                                              | ColumnQuerywithSchema | TagNotAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_Ratiogreaterthan05EmptyFalseTrue | email     | PostgressTag1,Postgress_BA,PostgreSQL,Email Address | ColumnQuerywithSchema | TagAssigned    |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_Ratiogreaterthan05EmptyFalseTrue | ipaddress | PostgressTag1,Postgress_BA,PostgreSQL,IP Address    | ColumnQuerywithSchema | TagAssigned    |
  ...
  Scenario Outline:SC#71b: Create Postgress Plugin config and start it
    ...
    #7089401#
  @positve @regression @sanity @PIITag
  Scenario:SC#71.b: Verify Tag is set for the column when dataPattern and minimumRatio(greater than 0.5) is passed which has a regexp that matches with the data in column in postgres table.
  (Ex: 0.6 - 6 or more rows should have matching column values including empty) - Match Empty is False -10 rows , 3 rows empty, 4 rows match,3 rows does not match.
    Given Tag verification of UI items in API for all the DataTypes
      | ClusterName             | ServiceName | DatabaseName | SchemaName | TableName/Filename                          | Column    | Tags                                                | Query                 | Action      |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_Ratiogreaterthan05EmptyFalseTrue | ssn       | PostgressTag1,Postgress_BA,PostgreSQL,SSN           | ColumnQuerywithSchema | TagAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_Ratiogreaterthan05EmptyFalseTrue | full_name | PostgressTag1,Postgress_BA,PostgreSQL,Full Name     | ColumnQuerywithSchema | TagAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_Ratiogreaterthan05EmptyFalseTrue | gender    | PostgressTag1,Postgress_BA,PostgreSQL,Gender        | ColumnQuerywithSchema | TagAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_Ratiogreaterthan05EmptyFalseTrue | email     | PostgressTag1,Postgress_BA,PostgreSQL,Email Address | ColumnQuerywithSchema | TagAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_Ratiogreaterthan05EmptyFalseTrue | ipaddress | PostgressTag1,Postgress_BA,PostgreSQL,IP Address    | ColumnQuerywithSchema | TagAssigned |
  ...
  (Ex: 0.6 - 6 or more rows should have matcning column values - dataPattern and minimumRatio passed).
    Given Tag verification of UI items in API for all the DataTypes
      | ClusterName             | ServiceName | DatabaseName | SchemaName | TableName/Filename                         | Column   | Tags                         | Query                 | Action         |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_Ratiogreaterthan05MatchFullTrue | comments | Fullmatch PII,Fullmatch PII1 | ColumnQuerywithSchema | TagNotAssigned |
  ...
  (Ex: 0.6 - 6 or more rows should have matcning column values - dataPattern and minimumRatio passed).
    Given Tag verification of UI items in API for all the DataTypes
      | ClusterName             | ServiceName | DatabaseName | SchemaName | TableName/Filename                         | Column   | Tags                                                               | Query                 | Action      |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_Ratiogreaterthan05MatchFullTrue | comments | PostgressTag1,PostgreSQL,Postgress_BA,Fullmatch PII,Fullmatch PII1 | ColumnQuerywithSchema | TagAssigned |
  ...
  (Ex: 0.2 - 2 or more rows should have matcning column values - namePattern,typePattern,dataPattern and minimumRatio passed).
    Given Tag verification of UI items in API for all the DataTypes
      | ClusterName             | ServiceName | DatabaseName | SchemaName | TableName/Filename                        | Column   | Tags                         | Query                 | Action         |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_Ratiolesserthan05MatchFullTrue | comments | Fullmatch PII,Fullmatch PII1 | ColumnQuerywithSchema | TagNotAssigned |
...
#######################################################################################################################################
  ...
  (Ex: 0.6 - 6 or more rows should have matcning column values including empty) - Match Empty is False -10 rows , 3 rows empty, 4 rows match,3 rows does not match
    Given Tag verification of UI items in API for all the DataTypes
      | ClusterName             | ServiceName | DatabaseName | SchemaName | TableName/Filename                              | Column    | Tags                                                | Query                 | Action         |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_Ratiogreaterthan05EmptyFalseTrueView | email     | PostgressTag1,Postgress_BA,PostgreSQL,Email Address | ColumnQuerywithSchema | TagAssigned    |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_Ratiogreaterthan05EmptyFalseTrueView | ipaddress | PostgressTag1,Postgress_BA,PostgreSQL,IP Address    | ColumnQuerywithSchema | TagAssigned    |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_Ratiogreaterthan05EmptyFalseTrueView | ssn       | SSN                                                 | ColumnQuerywithSchema | TagNotAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_Ratiogreaterthan05EmptyFalseTrueView | full_name | Full Name                                           | ColumnQuerywithSchema | TagNotAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_Ratiogreaterthan05EmptyFalseTrueView | gender    | Gender                                              | ColumnQuerywithSchema | TagNotAssigned |
  ...
  (Ex: 0.6 - 6 or more rows should have matching column values including empty) - Match Empty is False -10 rows , 3 rows empty, 4 rows match,3 rows does not match.
    Given Tag verification of UI items in API for all the DataTypes
      ...
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_Ratiogreaterthan05EmptyFalseTrueView | email     | PostgressTag1,Postgress_BA,PostgreSQL,Email Address | ColumnQuerywithSchema | TagAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_Ratiogreaterthan05EmptyFalseTrueView | ipaddress | PostgressTag1,Postgress_BA,PostgreSQL,IP Address    | ColumnQuerywithSchema | TagAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_Ratiogreaterthan05EmptyFalseTrueView | ssn       | PostgressTag1,Postgress_BA,PostgreSQL,SSN           | ColumnQuerywithSchema | TagAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_Ratiogreaterthan05EmptyFalseTrueView | full_name | PostgressTag1,Postgress_BA,PostgreSQL,Full Name     | ColumnQuerywithSchema | TagAssigned |
      | dechewindock01v.asg.com | PostgreSQL  | postgres     | tagschema  | TAGDETAILS_Ratiogreaterthan05EmptyFalseTrueView | gender    | PostgressTag1,Postgress_BA,PostgreSQL,Gender        | ColumnQuerywithSchema | TagAssigned |
  ...
  Scenario Outline: SC78#-MLP_24889_Verify PostgressAnalyzer analyzes multiple DB when more than one DB is cataloged by Cataloger.
    Given endpoint for "<ServiceName>" for "<ServiceUser>" having "<Header>" and query param with "<Query>" "<Param>" for request type "<type>" with url "<url>" and body as string from "<bodyFile>" with "<path>" and verify "<response code>" and "<response message>" using "<jsonPath>"
    Examples:
      | ServiceName | ServiceUser    | Header           | Query | Param | type         | url                                                                                                              | bodyFile                                                                            | path                                       | response code | response message                                    | jsonPath                                                                      |
      | IDC         | TestSystemUser | application/json |       |       | Put          | settings/analyzers/PostgreSQLDBDataSource/PostgresDS1                                                            | payloads/ida/postgressPayloads/DataSource/PostgresValidDataSourceConfig_DiffDB.json | $.PostgresDS1                              | 204           |                                                     |                                                                               |
      | IDC         | TestSystemUser | application/json |       |       | Get          | settings/analyzers/PostgreSQLDBDataSource/PostgresDS1                                                            |                                                                                     |                                            | 200           | PostgresDS1                                         |                                                                               |
      | IDC         | TestSystemUser | application/json |       |       | Put          | settings/analyzers/PostgreSQLDBDataSource/PostgresDS2                                                            | payloads/ida/postgressPayloads/DataSource/PostgresValidDataSourceConfig_DiffDB.json | $.PostgresDS2                              | 204           |                                                     |                                                                               |
      | IDC         | TestSystemUser | application/json |       |       | Get          | settings/analyzers/PostgreSQLDBDataSource/PostgresDS2                                                            |                                                                                     |                                            | 200           | PostgresDS2                                         |                                                                               |
      | IDC         | TestSystemUser | application/json |       |       | Put          | settings/analyzers/PostgreSQLDBDataSource/PostgressValidDataSource_TEST_DEFAULT_CONFIGURATION                    | payloads/ida/postgressPayloads/DataSource/PostgresValidDataSourceConfig_DiffDB.json | $.DefaultTestConnectionConfig              | 204           |                                                     |                                                                               |
      | IDC         | TestSystemUser | application/json |       |       | Get          | settings/analyzers/PostgreSQLDBDataSource/PostgressValidDataSource_TEST_DEFAULT_CONFIGURATION                    |                                                                                     |                                            | 200           | PostgressValidDataSource_TEST_DEFAULT_CONFIGURATION |                                                                               |
      | IDC         | TestSystemUser | application/json |       |       | Put          | settings/analyzers/PostgreSQLDBCataloger/PostgressPlugin_postgres_DB                                             | payloads/ida/postgressPayloads/DataSource/PostgresValidDataSourceConfig_DiffDB.json | $.PostgressPlugin_postgres_DB              | 204           |                                                     |                                                                               |
      | IDC         | TestSystemUser | application/json |       |       | Get          | settings/analyzers/PostgreSQLDBCataloger/PostgressPlugin_postgres_DB                                             |                                                                                     |                                            | 200           | PostgressPlugin_postgres_DB                         |                                                                               |
      | IDC         | TestSystemUser | application/json |       |       | RecursiveGet | extensions/analyzers/status/LocalNode/cataloger/PostgreSQLDBCataloger/PostgressPlugin_postgres_DB                |                                                                                     |                                            | 200           | IDLE                                                | $.[?(@.configurationName=='PostgressPlugin_postgres_DB')].status              |
      | IDC         | TestSystemUser | application/json |       |       | Post         | extensions/analyzers/start/LocalNode/cataloger/PostgreSQLDBCataloger/PostgressPlugin_postgres_DB                 |                                                                                     |                                            | 200           |                                                     |                                                                               |
      | IDC         | TestSystemUser | application/json |       |       | RecursiveGet | extensions/analyzers/status/LocalNode/cataloger/PostgreSQLDBCataloger/PostgressPlugin_postgres_DB                |                                                                                     |                                            | 200           | IDLE                                                | $.[?(@.configurationName=='PostgressPlugin_postgres_DB')].status              |
      | IDC         | TestSystemUser | application/json |       |       | Put          | settings/analyzers/PostgreSQLDBCataloger/PostgressPlugin_DVDRENTAL_DB                                            | payloads/ida/postgressPayloads/DataSource/PostgresValidDataSourceConfig_DiffDB.json | $.PostgressPlugin_DVDRENTAL_DB             | 204           |                                                     |                                                                               |
      | IDC         | TestSystemUser | application/json |       |       | Get          | settings/analyzers/PostgreSQLDBCataloger/PostgressPlugin_DVDRENTAL_DB                                            |                                                                                     |                                            | 200           | PostgressPlugin_DVDRENTAL_DB                        |                                                                               |
      | IDC         | TestSystemUser | application/json |       |       | RecursiveGet | extensions/analyzers/status/LocalNode/cataloger/PostgreSQLDBCataloger/PostgressPlugin_DVDRENTAL_DB               |                                                                                     |                                            | 200           | IDLE                                                | $.[?(@.configurationName=='PostgressPlugin_DVDRENTAL_DB')].status             |
      | IDC         | TestSystemUser | application/json |       |       | Post         | extensions/analyzers/start/LocalNode/cataloger/PostgreSQLDBCataloger/PostgressPlugin_DVDRENTAL_DB                |                                                                                     |                                            | 200           |                                                     |                                                                               |
      | IDC         | TestSystemUser | application/json |       |       | RecursiveGet | extensions/analyzers/status/LocalNode/cataloger/PostgreSQLDBCataloger/PostgressPlugin_DVDRENTAL_DB               |                                                                                     |                                            | 200           | IDLE                                                | $.[?(@.configurationName=='PostgressPlugin_DVDRENTAL_DB')].status             |
      | IDC         | TestSystemUser | application/json |       |       | Put          | settings/analyzers/PostgreSQLDBAnalyzer/PostgressAnalyzerDiffDatabaseTableFilter                                 | payloads/ida/postgressPayloads/DataSource/PostgresValidDataSourceConfig_DiffDB.json | $.PostgressAnalyzerDiffDatabaseTableFilter | 204           |                                                     |                                                                               |
      | IDC         | TestSystemUser | application/json |       |       | Get          | settings/analyzers/PostgreSQLDBAnalyzer/PostgressAnalyzerDiffDatabaseTableFilter                                 |                                                                                     |                                            | 200           | PostgressAnalyzerDiffDatabaseTableFilter            |                                                                               |
      | IDC         | TestSystemUser | application/json |       |       | RecursiveGet | extensions/analyzers/status/LocalNode/dataanalyzer/PostgreSQLDBAnalyzer/PostgressAnalyzerDiffDatabaseTableFilter |                                                                                     |                                            | 200           | IDLE                                                | $.[?(@.configurationName=='PostgressAnalyzerDiffDatabaseTableFilter')].status |
      | IDC         | TestSystemUser | application/json |       |       | Post         | extensions/analyzers/start/LocalNode/dataanalyzer/PostgreSQLDBAnalyzer/PostgressAnalyzerDiffDatabaseTableFilter  |                                                                                     |                                            | 200           |                                                     |                                                                               |
      | IDC         | TestSystemUser | application/json |       |       | RecursiveGet | extensions/analyzers/status/LocalNode/dataanalyzer/PostgreSQLDBAnalyzer/PostgressAnalyzerDiffDatabaseTableFilter |                                                                                     |                                            | 200           | IDLE                                                | $.[?(@.configurationName=='PostgressAnalyzerDiffDatabaseTableFilter')].status |


  @positve @regression @sanity @webtest
  Scenario:SC78#Verify analyzer does analysis for multiple db cataloged in catalog.
    Given User launch browser and traverse to login page
    When user enter credentials for "System Administrator1" role
    And user enters the search text "PostgressAnalyzerDiffDatabaseTableFilter" and clicks on search
    And user performs "facet selection" in "PostgressAnalyzerDiffDatabaseTableFilter" attribute under "Tags" facets in Item Search results page
    And user performs "facet selection" in "Table" attribute under "Metadata Type" facets in Item Search results page
    And user performs "item click" on "DiffDataTypes" item from search results
    Then user "verify metadata attributes" section has following values
      | metaDataAttribute  | widgetName |
    ...
    Then user performs click and verify in new window
      | Table   | value      | Action               | RetainPrevwindow | indexSwitch |
      | Columns | biginttype | click and switch tab | No               |             |
    Then user "verify metadata attributes" section has following values
      | metaDataAttribute  | widgetName |
      | Last catalogued at | Lifecycle  |
      | Last analyzed at   | Lifecycle  |
    ...
    Then user performs click and verify in new window
      | Table   | value         | Action               | RetainPrevwindow | indexSwitch |
      | Columns | charactertype | click and switch tab | No               |             |
    Then user "verify metadata attributes" section has following values
      | metaDataAttribute  | widgetName |
      | Last catalogued at | Lifecycle  |
      | Last analyzed at   | Lifecycle  |
    ...
    Then user performs click and verify in new window
      | Table | value                  | Action               | RetainPrevwindow | indexSwitch |
      | Table | DiffDataTypesMinimized | click and switch tab | No               |             |
    Then user "verify metadata attributes" section has following values
      | metaDataAttribute  | widgetName |
      | Last catalogued at | Lifecycle  |
      | Last analyzed at   | Lifecycle  |
    ...
    Then user performs click and verify in new window
      | Table   | value       | Action               | RetainPrevwindow | indexSwitch |
      | Columns | integertype | click and switch tab | No               |             |
    Then user "verify metadata attributes" section has following values
      | metaDataAttribute  | widgetName |
      | Last catalogued at | Lifecycle  |
      | Last analyzed at   | Lifecycle  |
    ...
    Then user performs click and verify in new window
      | Table   | value       | Action               | RetainPrevwindow | indexSwitch |
      | Columns | numerictype | click and switch tab | No               |             |
    Then user "verify metadata attributes" section has following values
      | metaDataAttribute  | widgetName |
      | Last catalogued at | Lifecycle  |
      | Last analyzed at   | Lifecycle  |
    ...
    Then user performs click and verify in new window
      | Table   | value         | Action               | RetainPrevwindow | indexSwitch |
      | Columns | timestamptype | click and switch tab | No               |             |
    Then user "verify metadata attributes" section has following values
      | metaDataAttribute  | widgetName |
      | Last catalogued at | Lifecycle  |
      | Last analyzed at   | Lifecycle  |
    ...
    Then user performs click and verify in new window
      | Table   | value        | Action               | RetainPrevwindow | indexSwitch |
      | Columns | smallinttype | click and switch tab | No               |             |
    Then user "verify metadata attributes" section has following values
      | metaDataAttribute  | widgetName |
      | Last catalogued at | Lifecycle  |
      | Last analyzed at   | Lifecycle  |
    ...
    Then user performs click and verify in new window
      | Table     | value     | Action               | RetainPrevwindow | indexSwitch |
      | Databases | dvdrental | click and switch tab | No               |             |
      | Schemas   | public    | click and switch tab | No               |             |
    ...
    Then user performs click and verify in new window
      | Table   | value       | Action               | RetainPrevwindow | indexSwitch |
      | Columns | last_update | click and switch tab | No               |             |
    Then user "verify metadata attributes" section has following values
      | metaDataAttribute  | widgetName |
      | Last catalogued at | Lifecycle  |
      | Last analyzed at   | Lifecycle  |
    ...
    Then user performs click and verify in new window
      | Table  | value   | Action               | RetainPrevwindow | indexSwitch |
      | Tables | country | click and switch tab | No               |             |
    Then user "verify metadata attributes" section has following values
      | metaDataAttribute  | widgetName |
      | Last catalogued at | Lifecycle  |
      | Last analyzed at   | Lifecycle  |
  ...
  Scenario Outline:SC79:user get the Dynamic ID's (Database ID) for the Schema "public,postgresstestschema" and Table "NATION,CUSTOMER,SCHOOL and snowflakecsvexttable1"
    ...
    Examples:
      ...
      | APPDBPOSTGRES | AsgScope_ID | Default | Schema | public              | city                   | payloads/ida/postgressPayloads/API/items.json | $.postgress.Tables.city                   |
      | APPDBPOSTGRES | AsgScope_ID | Default | Schema | public              | country                | payloads/ida/postgressPayloads/API/items.json | $.postgress.Tables.customer               |
      | APPDBPOSTGRES | AsgScope_ID | Default | Schema | postgresstestschema | DiffDataTypesMinimized | payloads/ida/postgressPayloads/API/items.json | $.postgress.Tables.DiffDataTypesMinimized |
      | APPDBPOSTGRES | AsgScope_ID | Default | Schema | postgresstestschema | DiffDataTypes          | payloads/ida/postgressPayloads/API/items.json | $.postgress.Tables.DiffDataTypes          |

  Scenario Outline: SC79:user hits the TableID's and save the DataSample Values
    Given A query param with "" and "" and supply authorized users, contentType and Accept headers
    ...
    Examples:
      ...
      | components/Default/definition/dataSample/Default.File:::dynamic | 200          | $.postgress.Tables.city                   | payloads/ida/postgressPayloads/API/items.json | payloads\ida\postgressPayloads\API\Actual\city.json                |            |
      | components/Default/definition/dataSample/Default.File:::dynamic | 200          | $.postgress.Tables.customer               | payloads/ida/postgressPayloads/API/items.json | payloads\ida\postgressPayloads\API\Actual\customer.json              |            |
      | components/Default/definition/dataSample/Default.File:::dynamic | 200          | $.postgress.Tables.DiffDataTypesMinimized | payloads/ida/postgressPayloads/API/items.json | payloads\ida\postgressPayloads\API\Actual\DiffDataTypesMinimized.json                |            |
      | components/Default/definition/dataSample/Default.File:::dynamic | 200          | $.postgress.Tables.DiffDataTypes          | payloads/ida/postgressPayloads/API/items.json | payloads\ida\postgressPayloads\API\Actual\DiffDataTypes.json |            |

#7152591
  Scenario: SC#79 MLP_24048_Verify the DataSamples values are as expected
    Then file content in "ida\postgressPayloads\API\Actual\city.json" should be same as the content in "ida\postgressPayloads\API\Expected\city.json"
    Then file content in "ida\postgressPayloads\API\Actual\customer.json" should be same as the content in "ida\postgressPayloads\API\Expected\customer.json"
    Then file content in "ida\postgressPayloads\API\Actual\DiffDataTypesMinimized.json" should be same as the content in "ida\postgressPayloads\API\Expected\DiffDataTypesMinimized.json"
    Then file content in "ida\postgressPayloads\API\Actual\DiffDataTypes.json" should be same as the content in "ida\postgressPayloads\API\Expected\DiffDataTypes.json"


  @sanity @positive @regression
  Scenario:SC#79_Delete Cluster , Cataloger and Analyzer Analysis file
    Given Delete multiple values in IDC UI with below parameters
      | deleteAction     | catalog | name                                | type     | query | param |
      | MultipleIDDelete | Default | dechewindock01v.asg.com             | Cluster  |       |       |
      | MultipleIDDelete | Default | cataloger/PostgreSQLDBCataloger/%   | Analysis |       |       |
      | MultipleIDDelete | Default | dataanalyzer/PostgreSQLDBAnalyzer/% | Analysis |       |       |
