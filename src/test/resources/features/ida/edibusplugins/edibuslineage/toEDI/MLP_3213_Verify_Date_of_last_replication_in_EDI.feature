Feature: MLP_3213_Verify_Date_of_last_replication_in_EDI

  @edibus @mlp-3213 @positive @release10.0
  Scenario Outline:SC1#To import oracle xml items to Default catalog
...
#5583179#
  ...
  @edibus @mlp-3213 @positive @release10.0
  Scenario Outline:MLP-3213_SC2#_To import items of all types
...
#5583178#
  ...
  @edibus @mlp-3213 @positive @release10.0
  Scenario Outline:SC3#_To import items of all types
    Given Delete multiple values in IDC UI with below parameters
      | deleteAction     | catalog | name               | type    | query | param |
      | SingleItemDelete | Default | LineageTestCluster | Cluster |       |       |
    Given endpoint for "<ServiceName>" for "<ServiceUser>" having "<Header>" and query param with "<Query>" "<Param>" for request type "<type>" with url "<url>" and body "<body>" and verify "<response code>" and "<response message>" using "<jsonPath>"
 ...
 #5583177#BugID MLP-17570#
  ...
  Scenario: MLP-3213_SC3#_Export items from IDX to EDI with incremental as true and validate deleted item is removed in EDI
    ...
    And user connects Rochade Server and "compare count" the items in EDI subject area
      | databaseName | subjectArea | subjectAreaVersion | query                                                                    |
      | AP-DATA      | AUTOMATION  | 1.0                | (XNAME * *  ~/ @* ),AND,( INSTCFGCNT > 0 ),AND,( TYPE = DWR_RDB_COLUMN ) |
    And user connects Rochade Server and "store date" the items in EDI subject area
      | databaseName | subjectArea | subjectAreaVersion | itemName | itemType       | operationName |
      | AP-DATA      | AUTOMATION  | 1.0                | Col1     | DWR_RDB_COLUMN | ModifiedDate  |
    ...
    Then METADATA widget should have following item values
      | metaDataItem     | metaDataItemValue |
      | Number of errors | 0                 |
    And user connects Rochade Server and "compare StoredDate" the items in EDI subject area
      | databaseName | subjectArea | subjectAreaVersion | itemName | itemType       | operationName |
      | AP-DATA      | AUTOMATION  | 1.0                | Col2     | DWR_RDB_COLUMN | ModifiedDate  |
    And user clicks on logout button
