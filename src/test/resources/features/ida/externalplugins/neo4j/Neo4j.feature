Feature:  Neo4j plugin for Enterprise Edition

  @positive @datacreation @MLP-8658
  Scenario:Create data in Neo4j
    Given User connects to neoDB "Enterprise Edition" and "Create" data
    Then user "verify presence" of following labels in neoDB "Enterprise Edition"
      | "player"     |
      | "Country"    |
      | "Tournament" |
      | "umpire"     |
  ...
  Scenario: Create Neo4j Plugin config1
    Given Execute REST API with following parameters
      | Header           | Query | Param | type | url                               | body                                        | response code | response message | jsonPath |
      | application/json | raw   | false | Put  | settings/analyzers/Neo4jCataloger | ida/neo4jPayloads/neo4jCatalogerConfig.json | 204           |                  |          |
      |                  |       |       | Get  | settings/analyzers/Neo4jCataloger |                                             | 200           | Neo4jCataloger   |          |

  @positive @MLP-8658
  Scenario: Start the plugin config1
    Given Execute REST API with following parameters
      | Header           | Query | Param | type         | url                                                                            | body | response code | response message | jsonPath                                            |
      | application/json |       |       | RecursiveGet | /extensions/analyzers/status/LocalNode/cataloger/Neo4jCataloger/Neo4jCataloger |      | 200           | IDLE             | $.[?(@.configurationName=='Neo4jCataloger')].status |
      |                  |       |       | Post         | /extensions/analyzers/start/LocalNode/cataloger/Neo4jCataloger/Neo4jCataloger  |      | 200           |                  |                                                     |
      |                  |       |       | RecursiveGet | /extensions/analyzers/status/LocalNode/cataloger/Neo4jCataloger/Neo4jCataloger |      | 200           | IDLE             | $.[?(@.configurationName=='Neo4jCataloger')].status |
  ...
  Scenario: Verify Neo4jCataloger collects items like Cluster, Service, Database, Table, Columns if node condition is specified in configuration.
    Given Execute REST API with following parameters
      ...
      |                  |       |       | RecursiveGet    | /extensions/analyzers/status/InternalNode/cataloger/Neo4jCataloger/Neo4jCatalogerWithNodeCondition |                                     | 200           | IDLE             | $.[?(@.configurationName=='Neo4jCatalogerWithNodeCondition')].status |              |               |
      |                  |       |       | Post            | /extensions/analyzers/start/InternalNode/cataloger/Neo4jCataloger/Neo4jCatalogerWithNodeCondition  |                                     | 200           |                  |                                                                      |              |               |
      |                  |       |       | RecursiveGet    | /extensions/analyzers/status/InternalNode/cataloger/Neo4jCataloger/Neo4jCatalogerWithNodeCondition |                                     | 200           | IDLE             | $.[?(@.configurationName=='Neo4jCatalogerWithNodeCondition')].status |              |               |
  ...
  Scenario: Verify the Database should have the index constraint inside techData in metadata information in IDC UI and Database
    ...
    Then user "verifies" the presence of the Values in "ida/neo4jPayloads/actual.json" json using the Jsonpath "$..Indexes"
      | Indexes1 | INDEX ON :Category(categoryID) |
      | Indexes2 | INDEX ON :Customer(customerID) |
      | Indexes3 | INDEX ON :Order(orderID)       |
      | Indexes4 | INDEX ON :Product(productID)   |
      | Indexes5 | INDEX ON :Supplier(supplierID) |
      | Indexes6 | INDEX ON :Category(isbn)       |
      | Indexes7 | INDEX ON :Order(isbn)          |
      | Indexes8 | INDEX ON :Product(isbn)        |
      | Indexes9 | INDEX ON :Supplier(isbn)       |
  ...
  Scenario:Update data in Neo4j
    Given User connects to neoDB "Enterprise Edition" and "update" data
    Then user "verify presence" of following labels in neoDB "Enterprise Edition"
      | "Album" |
  ...
  Scenario: Verify Neo4jCataloger plugin config does not catalog any items if the Repository Username is incorrectly provided
    Given Execute REST API with following parameters
      | Header           | Query | Param | type            | url                                                                                               | body                                | response code | response message | jsonPath                                                               | endpointType | itemName      |
      | application/json |       |       | DeleteAndCreate | settings/catalogs                                                                                 | ida/neo4jPayloads/neo4jCatalog.json | 204           |                  |                                                                        | catalog      | Neo4j CATALOG |
      |                  |       |       | RecursiveGet    | /extensions/analyzers/status/LocalNode/cataloger/Neo4jCataloger/Neo4jCatalogerWithInvalidUsername |                                     | 200           | IDLE             | $.[?(@.configurationName=='Neo4jCatalogerWithInvalidUsername')].status |              |               |
      |                  |       |       | Post            | /extensions/analyzers/start/LocalNode/cataloger/Neo4jCataloger/Neo4jCatalogerWithInvalidUsername  |                                     | 200           |                  |                                                                        |              |               |
      |                  |       |       | RecursiveGet    | /extensions/analyzers/status/LocalNode/cataloger/Neo4jCataloger/Neo4jCatalogerWithInvalidUsername |                                     | 200           | IDLE             | $.[?(@.configurationName=='Neo4jCatalogerWithInvalidUsername')].status |              |               |
    And User launch browser and traverse to login page
    And user enter credentials for "System Administrator1" role
    And user select "Neo4j CATALOG" from Catalog list
  ...
  Scenario: Verify Neo4jCataloger plugin config does not catalog any items if the repository password is incorrectly provided.
    Given Execute REST API with following parameters
      | Header           | Query | Param | type            | url                                                                                               | body                                | response code | response message | jsonPath                                                               | endpointType | itemName      |
      | application/json |       |       | DeleteAndCreate | settings/catalogs                                                                                 | ida/neo4jPayloads/neo4jCatalog.json | 204           |                  |                                                                        | catalog      | Neo4j CATALOG |
      |                  |       |       | RecursiveGet    | /extensions/analyzers/status/LocalNode/cataloger/Neo4jCataloger/Neo4jCatalogerWithInvalidPassword |                                     | 200           | IDLE             | $.[?(@.configurationName=='Neo4jCatalogerWithInvalidPassword')].status |              |               |
      |                  |       |       | Post            | /extensions/analyzers/start/LocalNode/cataloger/Neo4jCataloger/Neo4jCatalogerWithInvalidPassword  |                                     | 200           |                  |                                                                        |              |               |
      |                  |       |       | RecursiveGet    | /extensions/analyzers/status/LocalNode/cataloger/Neo4jCataloger/Neo4jCatalogerWithInvalidPassword |                                     | 200           | IDLE             | $.[?(@.configurationName=='Neo4jCatalogerWithInvalidPassword')].status |              |               |
    And User launch browser and traverse to login page
    And user enter credentials for "System Administrator1" role
    And user select "Neo4j CATALOG" from Catalog list
  ...
  Scenario: Verify Neo4jCataloger does not catalog any items if host URL is incorrect
    Given Execute REST API with following parameters
      | Header           | Query | Param | type            | url                                                                                              | body                                | response code | response message | jsonPath                                                              | endpointType | itemName      |
      | application/json |       |       | DeleteAndCreate | settings/catalogs                                                                                | ida/neo4jPayloads/neo4jCatalog.json | 204           |                  |                                                                       | catalog      | Neo4j CATALOG |
      |                  |       |       | RecursiveGet    | /extensions/analyzers/status/LocalNode/cataloger/Neo4jCataloger/Neo4jCatalogerWithInvalidHostURL |                                     | 200           | IDLE             | $.[?(@.configurationName=='Neo4jCatalogerWithInvalidHostURL')].status |              |               |
      |                  |       |       | Post            | /extensions/analyzers/start/LocalNode/cataloger/Neo4jCataloger/Neo4jCatalogerWithInvalidHostURL  |                                     | 200           |                  |                                                                       |              |               |
      |                  |       |       | RecursiveGet    | /extensions/analyzers/status/LocalNode/cataloger/Neo4jCataloger/Neo4jCatalogerWithInvalidHostURL |                                     | 200           | IDLE             | $.[?(@.configurationName=='Neo4jCatalogerWithInvalidHostURL')].status |              |               |
    And User launch browser and traverse to login page
    And user enter credentials for "System Administrator1" role
    And user select "Neo4j CATALOG" from Catalog list
...
#      | Service      |
  ...
  Scenario:Verify the Analysed Column data from Neo4j plugin
    ...
    Then user verify "presence of facets" with following values under "Type" section in item search results page
      ...
      | Cluster      |
    And user performs "facet selection" in "Table" attribute under "Type" facets in Item Search results page
    Then user gets the table data sample and validates the columns in neoDB "Enterprise Edition"
  ...
  Scenario:Verify the Analysed Table data from Neo4j plugin
    ...
    And user connects to neoDB "Enterprise Edition" gets the "neo4j05" table tech data and validates with db table "V_table"
  ...
  Scenario:Verify the Analysed Servcie data from Neo4j plugin
    ...
    And user connects to neoDB "Enterprise Edition" gets the "neo4j05" service tech data and validates with db table "V_Service"


  @positive @data @MLP-8658
  Scenario:Delete data in Neo4j
    Given User connects to neoDB "Enterprise Edition" and "Delete" data
